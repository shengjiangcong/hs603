...
from moveit_msgs.msg import RobotTrajectory, AttachedCollisionObject, PlanningScene
...
capture_quaternion = Quaternion(
    0.70711, 0.70711, 0, 0)  # Quaternion(0, 0, 0, 1)

# 拍照识别位置的六轴角度
capture_joint = [0.5559790730476379, -0.8841944336891174, 2.39966082572937, -
                 0.0009109065285883844, 1.624731421470642, 0.5558713674545288]
...
###################################################
        ...
        foreward_wall_pose.header.frame_id = 'world' 
        foreward_wall_pose.pose.position.x = 0.5
        foreward_wall_pose.pose.position.y = 0.0
        foreward_wall_pose.pose.position.z = 0.25
        foreward_wall_pose.pose.orientation.w = 1.0
        foreward_wall_size = [0.01, 3, 3]
        self.scene.add_box('foreward_wall', foreward_wall_pose, foreward_wall_size)
...
##############添加附着障碍#############
        ...
        self.scene.attach_box(self.end_effector_link,
                              tool1_id, tool1_pose, tool1_size)
        rospy.sleep(1)

##############添加附着障碍#############
        tool2_id = 'tool2'
        self.scene.remove_attached_object(self.end_effector_link, tool2_id)
        tool2_size = [0.015, 0.015, 0.16]
        ...
        tool2_pose.pose.position.z = 0.05
        tool2_pose.pose.orientation.w = 1.0
        self.scene.attach_box(self.end_effector_link,
                              tool2_id, tool2_pose, tool2_size)
...
##############添加附着障碍#############
        ...
        self.scene.attach_box(self.end_effector_link,
                              tool3_id, tool3_pose, tool3_size)
        rospy.sleep(1)
    ...
    def addbox(self, size, pose, name):
        ...
        backward_wall_pose.pose.orientation.w = 1.0
        self.scene.add_box(
            backward_wall_id, backward_wall_pose, backward_wall_size)
        rospy.sleep(1)
    ...
    def moveTo(self, x, y, z):
        ...
        while (point_num == 0 and  not rospy.is_shutdown()):
            print '规划ing'
            traj = self.arm.plan()
            point_num = len(traj.joint_trajectory.points)
            if (point_num > 0):
                # a表示目标位子的joint0的角度
                a = traj.joint_trajectory.points[point_num - 1].positions[0]
                if (a > 0):
                    break
                a = 0
                point_num = 0
...
##########添加两个盒子###################
    ...
    pose2_L = [box2_x, box2_y-0.1175,
               height_shuipin_collision + size2_L[2]/2.0]
    pose2_R = [box2_x, box2_y+0.1175,
               height_shuipin_collision + size2_R[2]/2.0]
    pose2_F = [box2_x-0.1175, box2_y,
               height_shuipin_collision + size2_F[2]/2.0]
    pose2_B = [box2_x+0.1175, box2_y,
               height_shuipin_collision + size2_B[2]/2.0]
    ...
    while not rospy.is_shutdown():
        ...
        blue_count = 0
        black_count = 0
        demo.arm.set_max_velocity_scaling_factor(0.5)
        for i in range(4):
            demo.moveJoint(capture_joint)
            if demo.pick(x, y, pick_black_height) == True:
                if i == 0:
                    demo.place(redStore[0], redStore[1],
                               redStore[2] + red_count*0.03)
                    red_count = red_count + 1
                elif i == 1:
                    demo.place(greenStore[0], greenStore[1],
                               greenStore[2] + green_count*0.03)
                    green_count = green_count + 1
                elif i == 2:
                    demo.place(blueStore[0], blueStore[1],
                               blueStore[2] + blue_count*0.03)
                    blue_count = blue_count + 1
                else:
                    demo.place(blackStore[0], blackStore[1],
                               blackStore[2] + black_count*0.03)
                    black_count = black_count + 1
        rate.sleep()
        timeVar = time.time()
        count  = count +1
        print '<---'+str(timeVar)+'------>' + str(count)
    demo.moveToHome()
    demo.shutdown()
